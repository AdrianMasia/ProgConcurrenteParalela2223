...
int main( int argc, char *argv[] ) {
  ...
  if( miId == 0 ) {
    ...
    for( i = 1; i < numProcs; i++ ) {
        MPI_Send( &vectorInicial[i * dimVectorLocal], dimVectorLocal, MPI_DOUBLE, i, 33,
         MPI_COMM_WORLD );
    }
    t2 = MPI_Wtime();
    tDis = t2 - t1;
  ...
  if( miId == 0 ) {
    sumaFinal = sumaLocal;
    double sumaOtroLocal;
    for( i = 0; i < numProcs - 1; i++ ) {
        MPI_Recv( &sumaOtroLocal, 1, MPI_DOUBLE, MPI_ANY_SOURCE,
            33, MPI_COMM_WORLD, &st );
        sumaFinal += sumaOtroLocal;
    }
  } else {
