...
int main( int argc, char *argv[] ) {
  ...
  inicializaVectorX ( vectorLocal, dimVectorLocal );
  ...
  // ... (G)
  double *vectorRecogida = NULL;
  vectorRecogida = (double *) malloc ( sizeof ( double ) * numProcs);
  inicializaVectorX( vectorRecogida, numProcs );

  MPI_Gather( &sumaLocal, 1, MPI_DOUBLE, vectorRecogida, 1,
        MPI_DOUBLE, 0, MPI_COMM_WORLD );

  if( miId == 0 ) {
    sumaFinal = 0;
    for( i = 0; i < numProcs; i++ ) {
        sumaFinal += vectorRecogida[ i ];
    }
  }
  ...
  if ( miId == 0) {
    ...
    // ... (I)
    printf( "Proc: %d , tSec vs tPar = %lf , tSec vs (tDis+tPar) = %lf\n",
